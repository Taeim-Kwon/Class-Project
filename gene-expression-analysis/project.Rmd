...
# BIOS 785 Project Report

## Taeim Kwon, Arthi Hariharan, Seowoo Kim

## 4/26/2023

Introduction

Here, we read in the files consisting of barcodes, features, and counts to set up a Seurat object. There were 29,733 features across 100,064 samples within the assay as there were 29,733 rows and 100,064 columns were made for the counts matrix.
...
The meta data was added to the Seurat object, and the steps for pre-processing were performed such as quality control, normalization, feature selection, and scaling. After pre-processing, there were 29,733 features and 98,287 samples found in the assay. Also PCA was run for a linear dimensional reduction.
...
We chose to use an elboplot to determine the dimensionality of the dataset and concluded that we would use 16 PC for UMAP. There were 34 communities formed from 0 to 33 in the UMAP. We also set a seed so that we can demonstrate the UMAP consistently.
...
Based on the metadata, we performed the automatic annotation. However, some cells that we would like to annotate were missing, so we would look for marker genes for each cluster that were not annotated with information from the metadata.
...
Comparing the marker genes expressed in each cell, we made a note for cell type allocation. For example, if KRT18 and KRT8, which are expressed in epithelial cells but not found in the other cells, were noted from a cluster, the cluster would be renamed as epithelial cells. All the results were recorded in an excel file for manual annotation.
...
The samples were finally annotated with the cell types that we found above. Although we were not able to reprouce the exactly same UMAP as the one in the paper due to the randomness in UMAP, we could see that B-cells, plasmablasts, and myeloid cells were close to each other in a triangle area. Also, the epithealial cells, mesenchymal cells, and endothelial cells were aprat from each other.
...
The feature plot was plotted for each marker gene: EPCAM and PECAM1 for epithelial cells, MKI67 for cycling cells, CD3D for T-cells, CD68 for myeloid cells, MS4A1 for B-cells, JCHAIN for plasmablasts, PDGFRB for mesenchymal cells. We can see that their expressed levels and the cells that they are corresponding to are matching when comparing the feature plot with the annotated UMAP.

```{r Feature plot 1 b}
FeaturePlot(project, features = c("EPCAM", "MKI67", "CD3D", "CD68", "MS4A1", "JCHAIN", "PECAM1", "PDGFRB"))
```

The UMAPs by subtypes and samples were generated to reproduce such figures in the paper after isolating the epithalial cells from the rest of the other cells.

```{r Reproduce figure 1 d and e}
